    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['workflow:defaultOperate:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['workflow:defaultOperate:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['workflow:defaultOperate:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['workflow:defaultOperate:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="defaultOperateList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="流程默认操作主键" align="center" prop="id" v-if="true"/>
...
<script>
import { listDefaultOperate, getDefaultOperate, delDefaultOperate, addDefaultOperate, updateDefaultOperate } from "@/api/workflow/defaultOperate";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        id: [
          { required: true, message: "流程默认操作主键不能为空", trigger: "blur" }
        ],
        opeType: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listDefaultOperate(this.queryParams).then(response => {
        this.defaultOperateList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDefaultOperate(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateDefaultOperate(this.form).then(response => {
            ...
            addDefaultOperate(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除流程默认操作编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delDefaultOperate(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('workflow/defaultOperate/export', {
        ...this.queryParams
      }, `defaultOperate_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
