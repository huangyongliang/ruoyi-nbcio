...
public class WfProcessServiceImpl extends FlowServiceFactory implements IWfProcessService {
    ...
    public TableDataInfo<WfDefinitionVo> selectPageStartProcessList(ProcessQuery processQuery, PageQuery pageQuery) {
        ...
        // 先获取所有流程定义
        ProcessDefinitionQuery allDefinitionQuery = repositoryService.createProcessDefinitionQuery()
            .latestVersion()
            .active()
        ...
        // 构建搜索条件
        ProcessUtils.buildProcessSearch(allDefinitionQuery, processQuery);
        
        // 获取所有符合条件的流程定义
        List<ProcessDefinition> allDefinitions = allDefinitionQuery.list();
        
        // 过滤出OA类型的流程
        List<ProcessDefinition> oaDefinitions = new ArrayList<>();
        for (ProcessDefinition definition : allDefinitions) {
            ...
            if(StrUtil.equalsAnyIgnoreCase(appType, "OA")) {
                oaDefinitions.add(definition);
            }
        }
        
        // 计算实际的总数（只计算OA类型的）
        long pageTotal = oaDefinitions.size();
        if (pageTotal <= 0) {
        ...
        int end = Math.min(offset + pageQuery.getPageSize(), oaDefinitions.size());
        List<ProcessDefinition> pagedDefinitions = offset < end ? 
            oaDefinitions.subList(offset, end) : new ArrayList<>();
        
        // 转换为VO对象
        List<WfDefinitionVo> definitionVoList = new ArrayList<>();
        for (ProcessDefinition processDefinition : pagedDefinitions) {
...
//        result.put("finished",true);
    ...
    private R startProcess(ProcessDefinition procDef, Map<String, Object> variables) {
        ...
        if(StringUtils.isNotEmpty(dataId)) {//自定义业务表单
            ...
            WfCallBackServiceI flowCallBackService = callbackRegistry.getCallback(serviceImplName);
        ...
        if(StringUtils.isNotEmpty(dataId)) {//自定义业务表单
            ...
            if(tasks!=null) {
    	        ...
    	        WfCallBackServiceI flowCallBackServiceafter = callbackRegistry.getCallback(serviceImplNameafter);
    	        // 流程处理完后，进行回调业务层
